...
GROUPS = [
    {
        ...
        "description": "Download any files from Excel URL / hyperlink columns — with optional rename.",
        "icon":        "📥",
        "accent":      C["cyan"],
        "tag":         "Excel · Download",
        "count":       "1 tool",
        "tools": [
            {
                ...
                "description": "Download PDF, images, or any file from Excel URLs or hyperlinks. Optionally rename using an ID column.",
                "icon":        "⚡",
                "accent":      C["cyan"],
                "tag":         "Excel · Download",
                "script":      os.path.join(SCRIPTS, "file_downloader.py"),
                "sample":      os.path.join(SAMPLES, "sample_pdf_download_rename.xlsx"),
                "docs": {
                    "what": "Reads the first sheet of an Excel file, finds URLs either as plain cell text or as Excel hyperlinks (e.g. a “VIEW POD” label with a real link underneath), and downloads each file — PDF, JPG, PNG, or other types. Optionally renames each file using an ID column (extension is kept/detected automatically).",
                    "when": "Use this for bulk downloads from Excel lists — POD images, PDFs, tracking docs, or any direct file URL — with or without renaming.",
                    "steps": [
                        ("Open the tool", "Click 'Launch →' on the File Downloader card."),
                        ("Select your Excel file", "The first sheet is always used (no sheet name needed)."),
                        ("Enter the URL / link column", "Exact header for the column with URLs or hyperlinks (e.g. 'POD IMAGE' or 'URL')."),
                        ("Optional rename", "Tick 'Rename…' and enter an ID column (e.g. 'AWB NUMBER') to save as <ID>.jpg / <ID>.pdf."),
                        ("Click 'Start Download'", "Progress and status appear in the log. The output folder opens when done."),
                    ],
                    "tips": [
                        "Hyperlink cells work even when the visible text is not a URL (like VIEW POD).",
                        "File type is taken from the URL, Content-Type, or Content-Disposition — not forced to PDF.",
                        "Duplicate names get a _1, _2 suffix so nothing is overwritten.",
                        "Each run creates a new timestamped folder under Desktop\\OUTPUT\\File_Downloads.",
                    ],
                    "output": "Desktop\\OUTPUT\\File_Downloads\\YYYY-MM-DD_HH-MM-SS\\",
                },
            },
        ],
    },
    ...
    },
    {
        ...
        "tools": [
                ...
                {
                    "title":       "PDF Split & Rename",
                    "sample":      None,
                    "description": "Split multi-page PDFs into page groups and rename by regex (page text) or Excel column order.",
                    "icon":        "✂️",
                    "accent":      C["purple"],
                    "tag":         "PDF · Split",
                    "script":      os.path.join(SCRIPTS, "split_and_rename.py"),
                    "docs": {
                        "what": "Splits multi-page PDFs into fixed page groups (default 2). Rename mode A: regex on the first page text (e.g. SL numbers). Mode B: Excel order — each split is named from the next row in a selected column (1st split ← row 1, 2nd ← row 2, …). No page OCR/text needed in Excel mode.",
                        "when": "Use regex when the ID is printed on the page. Use Excel order when pages have no readable ID but you have a list in the same order as the PDF splits.",
                        "steps": [
                            ("Open the tool", "Click 'Launch →' on the PDF Split & Rename card under PDF Tools."),
                            ("Select PDF(s)", "Use 'File…' or 'Folder…'."),
                            ("Choose rename mode", "From page text (regex) or From Excel (order)."),
                            ("Regex mode", "Keep or edit the ID pattern (default \\bSL\\d+\\b)."),
                            ("Excel mode", "Browse an Excel file and pick the rename column. Row order must match split order."),
                            ("Set pages per file", "Default is 2."),
                            ("Click Split & Rename", "Outputs go to split_pdfs with split_rename_log.csv."),
                        ],
                        "tips": [
                            "Excel mode only renames by order — PDF page content is ignored.",
                            "If there are more splits than Excel rows, extras become Unknown_N.pdf.",
                            "Duplicate names get a numeric suffix (_1, _2).",
                        ],
    ...
    },
    {
        ...
        "tools": [
                ...
                },
                {
                    ...
                    "description": "Transform RBL Excel with name_1 / address_1 … → barcodes → pivot → split.",
                    "icon":        "🏦",
                    "accent":      C["blue"],
                    "tag":         "Excel · RBL",
                    "script":      os.path.join(SCRIPTS, "rbl_excel_transformer.py"),
                    "docs": {
                        "what": "RBL-format pipeline. Expects Excel/CSV with numbered person columns (name_1, address_1, name_2, address_2, …), unpivots each person into sticker rows, inserts barcodes, pivots by Loan Account No, then splits by address count.",
                        "when": "Use when your RBL / notice file is prepared with name_1 / address_1 style columns (not the old CUSTOMER NAME + Co-App Name layout).",
                        "steps": [
                            ("Prepare columns", "Rename person fields to name_1, address_1, name_2, address_2, … (optional: pin_1, mobile_1, …)."),
                            ("Select data file", "Browse .csv / .xlsx and pick the Data sheet."),
                            ("Select barcode file", "Pick barcode.xlsx or the Barcodes sheet from the sample workbook."),
                            ("Run Pipeline", "Outputs main.xlsx, sticker.xlsx, pivoted_data_with_unique_id.xlsx, and address_count_N.xlsx."),
                        ],
                        "tips": [
                            "Required pattern: name_1 + address_1 at minimum. Add name_2 / address_2 for co-apps.",
                            "final_add_N is accepted as an alias for address_N.",
                            "Group key is Loan Account No (stored as ref_no for pivot).",
                        ],
