...
PARAS_PRINT_SERVICES = [
    ...
    {
        "title":       "Network Data Builder",
        "description": "VLOOKUP branch address, name, pincode & contact from the Network list into a data.xlsx for Grouped Excel.",
        "icon":        "🗺️",
        "accent":      C["green"],
        "tag":         "Excel · VLOOKUP",
        "script":      os.path.join(PP_SCRIPTS, "network_data_builder.py"),
        "sample":      None,
        "docs": {
            "what": "Takes a PFL/roster Excel and a Network List Excel. Extracts the first 4 characters of each Location Code (LEFT formula), then looks up Address, Pin code, Branch Champion, and Branch Champion Contact. You enter product name (default ID CARD) and challan no in the UI. Writes data.xlsx with Sr No and ID Count left blank — then run Grouped Excel.",
            "when": "Use this as Step 1 of the Paras Print challan workflow — before Grouped Excel — whenever you need to enrich employee rows with branch champion and address details from the network master list.",
            "steps": [
                ("Select PFL / roster Excel", "Browse to the source .xlsx that has Location Code values like 2353_PFL. PFL column mapping auto-fills — change dropdowns if needed."),
                ("Select Network List Excel", "Browse to the network master .xlsx. Lookup & mapping columns auto-fill (Address, Branch Champion, Pin code, Contact) — change any dropdown if headers differ."),
                ("Enter product name & challan no", "Product name defaults to 'ID CARD'. Enter the challan number to apply to every row."),
                ("Click Build data.xlsx", "The tool matches LEFT(Location Code, 4) to the network lookup key and writes data.xlsx."),
                ("Review data.xlsx", "Open the output file to check. Leave Sr No and ID Count empty."),
                ("Run Grouped Excel", "Use the Grouped Excel tool on data.xlsx to join Employee Codes and add ID Count."),
            ],
            "tips": [
                "Location codes like 6418_PFL become 6418 for the lookup — same as Excel LEFT(col, 4).",
                "Unmatched location codes are listed in unmatched_locations.xlsx in the same output folder.",
                "All lookup/mapping columns are shown after you browse a file — pick a different column from the dropdown if auto-detect is wrong.",
                "Legal Entity and Pincode are optional — set to (none) to skip them.",
                "Output columns match the challan data template: Consignee Name, Legal Entity, address, pincode, Contact. No., Employee Code, location code.",
            ],
            "output": "Desktop\\OUTPUT\\Network_Data_Builder\\<timestamp>\\data.xlsx",
        },
    },
    {
        "title":       "PDF to JPG",
    ...
    {
        "title":       "B&W Converter",
        "description": "Remove background with AI, then convert subject to grayscale — output is a transparent PNG cutout.",
        "icon":        "🔲",
        "accent":      C["sky"],
        "tag":         "Photos",
        ...
        "sample":      None,
        "docs": {
            "what": "Two-step pipeline per image: (1) rembg AI removes the background, producing a transparent RGBA cutout; (2) the visible subject area is converted to grayscale while the alpha channel is preserved exactly. Output is always a transparent PNG — no white or black fill.",
            "when": "Use this when you want a black-and-white version of a photo with the background fully removed — for ID cards, stickers, layered designs, or any use where a transparent B&W cutout is needed.",
            "steps": [
                ...
                ("Click Run", "Click '▶  Remove BG & Convert to B&W'. Output PNGs are saved to the timestamped folder."),
            ],
            "tips": [
                "Requires Pillow (pip install Pillow) and rembg (pip install rembg).",
                "rembg downloads its AI model on first run — takes a moment but only once.",
                "Output is always PNG to preserve the transparent alpha channel.",
                "Works on .png, .jpg, .jpeg, .webp, .bmp, .tiff inputs.",
                "If you only need to change the background colour (not go B&W), use the BG Changer tool instead.",
            ],
